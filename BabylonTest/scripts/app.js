        ...
        function main() {
            ...
            var vm = new Vue({
                ...
                ready: function () {
                    ...
                    for (var i = 1; i < 9; i++) {
                        if (i <= 6) {
                            BABYLON.EDITOR.SceneFactory.AddYardContainer(core, containerNo++, block, 20, yardLocations[i], new BABYLON.Color3(Math.random(), Math.random(), Math.random()));
                        }
                    }
                ...
                methods: {
                    addContainer: function () {
                        ...
                        if (yardLocation) {
                            var con = BABYLON.EDITOR.SceneFactory.AddYardContainer(core, containerNo++, block, 20, yardLocation, new BABYLON.Color3(Math.random(), Math.random(), Math.random()));
                        }
                        else {
...
var BABYLON;
(function (BABYLON) {
    var yardBlock = (function (_super) {
        __extends(yardBlock, _super);
        function yardBlock() {
    ...
    var yardContainer = (function (_super) {
        ...
        Object.defineProperty(yardContainer.prototype, "yardLocation", {
            ...
            set: function (theValue) {
                ...
                this.position = new BABYLON.Vector3((theValue.row_x - 1 / 2) * this.scaling.x + block._boundingInfo.minimum.x, this.scaling.y / 2, (theValue.column_z - 1 / 2) * this.scaling.z + block._boundingInfo.minimum.z);
            },
            enumerable: true,
            configurable: true
        });
    ...
    (function (EDITOR) {
        var SceneFactory = (function () {
            ...
            */
            ...
            ////    return water;
            ////}
            SceneFactory.AddYardContainer = function (core, id, block, size, yardLocation, color) {
                var yContainer = BABYLON.Mesh.CreateBox("yardContainer" + id, 1, core.currentScene, false);
            ...
            SceneFactory.AddYardBlockGroundMesh = function (core, id, containerSize, columns, rows, levels) {
                var scene = core.currentScene;
                ...
                for (var r = 1; r <= rows; r++)
                    for (var c = 1; c <= columns; c++) {
                        ...
                        //slots
                        var containerSlot = BABYLON.Mesh.CreateBox("containerSlot" + id, 1, core.currentScene, false);
                        containerSlot.parent = tiledGround;
                        containerSlot.scaling = new BABYLON.Vector3(8, 8.6, containerSize);
                        containerSlot.position = new BABYLON.Vector3((r - 1 / 2) * containerSlot.scaling.x + xmin, containerSlot.scaling.y / 2, (c - 1 / 2) * containerSlot.scaling.z + zmin);
                        var containerMaterial = new BABYLON.StandardMaterial("containerMaterial", core.currentScene);
                        containerMaterial.diffuseColor = new BABYLON.Color3(0.4, 0.4, 0.4);
                        containerMaterial.specularColor = new BABYLON.Color3(0.4, 0.4, 0.4);
